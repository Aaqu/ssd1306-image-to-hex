class screen {
    ...
    create() {
        ...
        document.querySelector('#hex').addEventListener('click', () => {
            ...
            const hexGroups = this.screen.width * this.screen.height / 8;
            for (let hexGroup = 0; hexGroup < hexGroups; hexGroup++) {
                let pixelBinary = '';
                for (let i = startHexFomPixel; i <= startHexFomPixel + 7; i++) {
                    const pixelColor = rgb2hex(document.querySelector('#p' + i).style.backgroundColor);
            ...
            for (let y = 0; y < this.screen.height; y++) {
                for (let x = 0; x < this.screen.width / 8; x++) {
            ...
            const screenArea = this.screen.parent;
            screenArea.querySelector('.outHex').innerText = outString.slice(0, -2);
        });
